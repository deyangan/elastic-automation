...
- **The internal CA certificate** (`AD_domain_ca.pem`) used for LDAPS — also
...
- The internal domain and AD domain were replaced with `example.com` /
  `ad.example.com` throughout.
- Real server hostnames were replaced with descriptive placeholders
  (`es-prod-node4.example.com`, etc.) that preserve the cluster topology so
